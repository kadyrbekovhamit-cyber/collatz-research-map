...
Narration: edge-tts, en-US-JennyNeural, rate -3%, natural pitch. No voice cloning.
Only the intended narration text is sent to the online speech service.
Subtitles use the service's real word timing events. Client version, raw events,
script and generation metadata are retained with the video source.
No audio playback is used in local quality checks.

AI assistance: drafting, diagram implementation and internal argument review.
...
## New colour tour, 18 September 2026

- Public graph.json: unchanged data snapshot, 356 nodes / 532 edges. Category colour is an editorial annotation of existing node labels, not a confidence score.
- PowerBarrier label (R184) explicitly records a critical error in the earlier iteration; coral means this documented correction.
- Blue: project record marked internally checked within its assumptions. Cyan: selected explicit published-reference nodes CZKnown and NishiokaKnown. Amber: unresolved. No blanket novelty claim.
- Family relationship and several months of AI-assisted work: directly supplied by the user on 18 September 2026; attributed author statement, not independently audited.
- Main visual is a source-data rendering, not a recording of an unbuilt product.
